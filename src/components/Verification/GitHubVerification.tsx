...
import { verifyByGitHub } from "@/api/verification/github";
...
type GitHubVerificationProps = {
    token: string;
    code: string;
};

const GitHubVerification: FC<GitHubVerificationProps> = ({ code, token }) => {
    const mutation = useMutation({
        mutationFn: () => verifyByGitHub(code, token),
        onSuccess(data) {
            logger.debug(GitHubVerification.name, data);
        },
    });
