    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            .then(res => {
                ...
                state.guilds = [];
                const gs = res.data.guilds.filter(g => {
                    return <?= json_encode(array_map(fn($e) => $e . '', array_keys($_SESSION['tokens']))) ?>.includes(g.id);
                });

                console.log(res.data.guilds, gs);

                for (let index in els) {
                    console.log(index);
                    state.guilds.push(gs[index]);

                    if (res.data.guilds[index].id === '<?= $guild_id1 ?>') {
                        state.currentGuild = gs[index];
                        els[index].classList.add('active');
                        console.log('here');
                    }

                    els[index].innerHTML = gs[index].name;
                }

                if (!state.currentGuild) { 
                    console.log(state.guilds);
                    state.currentGuild = state.guilds.find(g => <?= json_encode(array_map(fn($e) => $e . '', array_keys($_SESSION['tokens']))) ?>.map(e => e.trim()).includes(g.id));
